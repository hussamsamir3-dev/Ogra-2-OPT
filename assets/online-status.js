...
(() => {
  ...
  function paint(){
    const [s,detail]=state(),el=badge(),text=copy(s),open=el.classList.contains('open');el.className=s+(open?' open':'');
    el.querySelector('b').textContent=text[0];el.querySelector('small').textContent=detail||text[1];el.title=detail||text[1];
  }
  const css=document.createElement('style');css.textContent=`
  #ogNetStatus{position:fixed!important;top:auto!important;right:max(10px,env(safe-area-inset-right))!important;left:auto!important;bottom:max(10px,env(safe-area-inset-bottom))!important;z-index:2147483700;display:flex;align-items:center;gap:8px;max-width:min(88vw,330px);min-height:35px;padding:7px 11px;border:1px solid rgba(255,255,255,.16);border-radius:999px;background:rgba(7,13,24,.9);color:#eef6ff;box-shadow:0 8px 26px rgba(0,0,0,.34);backdrop-filter:blur(12px);font:700 11px Cairo,system-ui,sans-serif;cursor:pointer;text-align:start;transition:.2s transform,.2s border-color}
